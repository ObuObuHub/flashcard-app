...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        setSuccess(true)
        // If email confirmation is disabled, redirect to decks
        if (data.session) {
          router.push('/decks')
          router.refresh()
        }
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-600 dark:text-green-400 px-4 py-3 rounded">
                ...
                Te rugăm să verifici inbox-ul și să dai click pe linkul de confirmare.
              </div>

              <div className="text-center text-sm text-gray-600 dark:text-gray-400">
                Poți închide această pagină și să te autentifici după ce ți-ai confirmat emailul.
              </div>
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-4">
          ...
          <div className="text-center">
            ...
            <CardDescription>
              Creează un cont gratuit pentru a începe învățarea
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <Button
              ...
              disabled={loading}
            >
              {loading ? 'Se încarcă...' : t.signupButton}
            </Button>
          </form>
